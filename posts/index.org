...
Lately, I have been drawn to VS Code again. And I like it.
...
* PUBLISHED New laptop arrived                                         :tech:
...
- The [[https://docs.microsoft.com/windows/package-manager/winget/][winget]] package manager package manager works like a charm. I also installed apps through the store, just to experience the difference, but winget has a much wider choice. The weird thin is, that removing an app must be done by selecting "Uninstall" in the start menu...
...
- Migrating [[https://www.gnu.org/software/emacs/][Emacs]] was fairly painless. I just needed to set =$HOME= and install [[https://www.orgroam.com/manual.html#C-Compiler-for-Windows][MSys2]] for Org-roam. This latter also removed my need for [[https://www.cygwin.com/][cygwin]].
...
* PUBLISHED Emacs + GPG = EasyPG                                       :tech:
CLOSED: [2022-06-06 Mon 11:09]

I looked at [[https://keepass.info/][KeePass]] to store confidential information on Windows. But than it occured to me that org files can easily be encrypted, so I used that instead.

Just add the extension =.gpg= to your file (so =vault.org= becomes =vault.org.gpg=) and your good to go. The double extension lets Emacs know its both encrypted and an org file.

I love how org mode turns out to be the perfect PIM with every new step.

And I was glad to find that EasyPG worked out of the box even on Windows.

* PUBLISHED Website monitoring                                          :dev:
CLOSED: [2022-06-05 Sun 12:20]

I maintain a number of websites for different clients and need to monitor their status. At this point a simple check if the site is still up would be sufficient, so I would prefer something simple. Other requirements: open source, self-hosted, configurable, web dashboard, automatic alerte and preferably available through docker compose.
